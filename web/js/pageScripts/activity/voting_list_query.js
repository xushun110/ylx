jQuery(function(){
    ...
    function _initData () {
        ...
        jQuery.ajax({
            dataType: "json",
            url: webBasePath + '/voting/findVotingList',
            data: queryParams,
            type: "POST",
            success: function (result) {
                if (result.code == 1) {
                    ...
                    if (result.data != null && result.data.data.length > 0) {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            ...
                            _html.push('<td>' + obj.keyword + '</td>');
                            _html.push('<td>' + obj.exp + '</td>');
                            _html.push('<td>' + obj.goldCoin + '</td>');
                            _html.push('<td>' + obj.endTime + '</td>');
                            _html.push('<td>' + obj.createTime + '</td>');;

                            var _operHtml = [];
                            _operHtml.push('<a href="/voteItem/goList/'+dataId+'" class="btn btn-primary btn-xs">投票选项</a>');
                            _operHtml.push('<a href="/voting/goEdit/' + dataId + '" class="btn btn-primary btn-xs">修改</a>');
