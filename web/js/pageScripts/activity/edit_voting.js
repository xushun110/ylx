jQuery(function(){
    ...
    /** 渲染道具下拉列表，上一个列表选中的，在下一个列表不显示 **/
    function _renderPropsAllSelect($ele){
        var $allProps = $(".props-list").find("select");
        $allProps.each(function(i,v){
            var $currSelect = $(this);
            //从第二行开始
            if(i>0){
                var $prevAllList = $currSelect.parent().parent().prevAll();
                //获取上面所有下拉列表
                if($prevAllList.length != 0){
                    $prevAllList.each(function(){
                        var $that = $(this);
                        var $prevSelect = $that.find('select');
                        var prevVal = $prevSelect.val();
                        //删除当前下拉，在上一下拉列表选中的值
                        $currSelect.find('option[value='+prevVal+']').remove();
                    });
                }
            }
        });
    ...
    function _setAjaxData () {
        ajaxdata.voteId = $('#dataId').val().trim();
